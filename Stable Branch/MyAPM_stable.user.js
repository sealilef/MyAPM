...
// @homepageURL  https://github.com/sealilef/MyAPM/blob/main/Stable%20Branch/MyAPM_stable.user.js
// @supportURL   https://github.com/sealilef/MyAPM/blob/main/Stable%20Branch/MyAPM_stable.user.js
// @updateURL    https://raw.githubusercontent.com/sealilef/MyAPM/main/Stable%20Branch/MyAPM_stable.user.js
// @downloadURL  https://raw.githubusercontent.com/sealilef/MyAPM/main/Stable%20Branch/MyAPM_stable.user.js
// @grant        GM_getValue
// @grant        GM_setValue
// @grant        unsafeWindow
...
(function () {
    ...
    const SCRIPT_BLOB_PATH = '/sealilef/MyAPM/blob/main/Stable Branch/MyAPM_stable.user.js';
    ...
    function maybeAutoClickGithubRaw() {
        ...
        const clickRaw = () => {
            ...
            const target = links.find((el) => {
                ...
                return text === 'Raw' || href.includes('/raw/refs/heads/main/Stable%20Branch/MyAPM_stable.user.js');
            });
    ...
    if (maybeRelaunchConflictingThemeUrl()) return;
    ...
    const THEME_ORIGINAL_CSS_PATH_KEY = '__myapmOriginalCssPath';
    const THEME_ORIGINAL_BEFORELOAD_KEY = '__myapmOriginalBeforeLoad';
    const THEME_BEFORELOAD_WRAPPER_KEY = '__myapmBeforeLoadWrapper';
    ...
    const THEME_OPTIONS = [
        ...
        { value: 'theme-hex-dark', label: 'Hex Dark' }
    ];
    const DARK_THEME_VALUES = new Set(['dark', 'theme-dark', 'theme-hex-dark']);
    const themeState = {
        activeTheme: null,
        bound: false,
        applying: false
    ...
    function readThemeStorage(key) {
        try {
            ...
            const topValue = topStorage ? topStorage.getItem(key) : null;
            if (topValue != null && topValue !== '') return String(topValue);
        } catch (_) {}
        try {
    ...
    function writeThemeStorage(key, value) {
        ...
        try {
            const topStorage = PAGE_WINDOW.top && PAGE_WINDOW.top.localStorage ? PAGE_WINDOW.top.localStorage : null;
            if (topStorage) topStorage.setItem(key, normalizedValue);
        } catch (_) {}
        try {
            localStorage.setItem(key, normalizedValue);
        } catch (_) {}
        try {
            if (typeof GM_setValue === 'function') GM_setValue(key, normalizedValue);
        } catch (_) {}
    }
    ...
    function applyThemeVisualHints(themeValue) {
        ...
        if (dark) {
            if (!flashStyle) {
                ...
                flashStyle.id = THEME_FLASH_STYLE_ID;
                (document.head || document.documentElement).appendChild(flashStyle);
            }
            flashStyle.textContent = 'html, body, .x-body, .x-viewport { background-color: #222 !important; color-scheme: dark !important; }';
        } else if (flashStyle) {
            flashStyle.remove();
        }
    ...
    function clearThemeArtifacts(doc = document) {
        [THEME_STYLE_ID, THEME_FLASH_STYLE_ID].forEach((id) => {
            const node = doc.getElementById(id);
            if (node) node.remove();
        });
        delete doc.documentElement.dataset.myapmTheme;
    }
    ...
    function enforceThemeLinks(themeValue, doc = document) {
        ...
        doc.querySelectorAll('link[rel="stylesheet"]').forEach((node) => {
            ...
            nextHref = nextHref.replace(/\b(neptune|crisp|triton)\b/i, themeSlug);
            if (nextHref && nextHref !== href) node.href = nextHref;
        });
    }
    ...
    function hookThemeTargets(targetWindow) {
                ...
                try {
                    ...
                    extObj[THEME_BEFORELOAD_WRAPPER_KEY] = function(...args) {
                        ...
                        if (typeof originalBeforeLoad === 'function') {
                            try {
                                return originalBeforeLoad.apply(this, args);
                            } catch (_) {}
                        }
                        return undefined;
                    };
                    Object.defineProperty(extObj, 'beforeLoad', {
                        configurable: true,
                        enumerable: true,
                        get: () => extObj[THEME_BEFORELOAD_WRAPPER_KEY],
                        set: (nextValue) => {
                            try { extObj[THEME_ORIGINAL_BEFORELOAD_KEY] = nextValue; } catch (_) {}
                        }
                    });
                } catch (_) {}
        ...
        try {
            patchEam(targetWindow.EAM);
            patchExt(targetWindow.Ext);

            let eamRef = targetWindow.EAM;
            Object.defineProperty(targetWindow, 'EAM', {
                configurable: true,
                enumerable: true,
                get: () => eamRef,
                set: (nextValue) => {
        ...
        try {
            let extRef = targetWindow.Ext;
            Object.defineProperty(targetWindow, 'Ext', {
                configurable: true,
                enumerable: true,
                get: () => extRef,
                set: (nextValue) => {
    ...
    function getThemeControllerWindow() {
        try {
            const topWin = PAGE_WINDOW.top || window.top || window;
            if (topWin && typeof topWin.__myapmApplyPreferredTheme === 'function') return topWin;
        } catch (_) {}
        return window;
    }
    ...
    function initThemeSystem() {
        ...
        try {
            window.__myapmApplyPreferredTheme = (themeValue) => {
                applyPreferredTheme(themeValue);
            };
        } catch (_) {}
    ...
    function buildWorkOrderUrl(workOrderNum, userFunction = 'WSJOBS') {
        ...
        if (!value) return '';
        return `${getTopOrigin()}/web/base/logindisp?tenant=AMAZONRMENA_PRD&FROMEMAIL=YES&SYSTEM_FUNCTION_NAME=WSJOBS&USER_FUNCTION_NAME=${encodeURIComponent(userFunction)}&workordernum=${encodeURIComponent(value)}`;
    }
    ...
    function buildAuditUrl(auditNum, workOrderNum = '') {
        ...
        return `${getTopOrigin()}/web/base/logindisp?tenant=AMAZONRMENA_PRD&FROMEMAIL=YES&SYSTEM_FUNCTION_NAME=WSJOBS&USER_FUNCTION_NAME=ADJOBS&workordernum=${encodeURIComponent(woValue)}&apmAudit=${encodeURIComponent(auditValue)}#apmAudit=${encodeURIComponent(auditValue)}`;
    }
    ...
    function createThemeSettingsCard() {
        const card = document.createElement('section');
        card.className = 'myapm-settings-card';
        Object.assign(card.style, {
            display: 'flex',
        ...
        const syncStatus = () => {
            ...
            try {
                const controllerWindow = getThemeControllerWindow();
                if (controllerWindow && controllerWindow !== window && controllerWindow.__myapmThemeState && controllerWindow.__myapmThemeState.activeTheme) {
                    selected = normalizeThemeValue(controllerWindow.__myapmThemeState.activeTheme);
                }
            } catch (_) {}
    ...
    function resolvePtpDescription(url, requestBody, responseObj) {
      ...
      for (const candidate of candidates) {
        try {
          ...
          const dashIndex = organization.indexOf(' - ');
          if (dashIndex > -1) {
            const description = cleanText(organization.slice(dashIndex + 3));
            if (description) return description;
          }
          const commaIndex = organization.indexOf(',');
          if (commaIndex > -1) {
            const description = cleanText(organization.slice(commaIndex + 1).replace(/^Title:\s*/i, ''));
            if (description) return description;
          }
        } catch (_) {}
      }
